  ...
  const buildDoctorStatus = async (userId?: string) => {
    ...
    return {
      ...
      trade_controls: {
        ...
        min_profit_pct: Math.max(DOCTOR_TAKE_PROFIT_PCT, getDoctorEffectiveControlNumber("min_profit_pct", Number(doctorRuntime.controls.min_profit_pct || 0.1))),
        stop_loss_pct: Math.max(DOCTOR_STOP_LOSS_PCT, getDoctorEffectiveControlNumber("stop_loss_pct", Number(doctorRuntime.controls.stop_loss_pct || 0.1))),
