...
const DOCTOR_MIN_LIQUIDITY_USD = 2500;
const DOCTOR_MIN_VOLUME_5M_USD = 5000;
const DOCTOR_MAX_TOKEN_AGE_SECONDS = 120;
const DOCTOR_MAX_TOKEN_AGE_MINUTES = 240;
const DOCTOR_TRADE_COOLDOWN_SECONDS = 180;
...
export async function registerRoutes(
  ...
  const applyDoctorRuntimeSnapshot = (loaded: Record<string, any>) => {
    ...
    doctorRuntime.controls.max_token_age_minutes = Math.min(
      DOCTOR_MAX_TOKEN_AGE_MINUTES,
      Math.max(Number(doctorRuntime.controls.min_token_age_minutes || 0), Number(doctorRuntime.controls.max_token_age_minutes || 10)),
    );
  ...
  const executeDoctorCycle = async (trigger: "manual" | "auto" = "manual", userId?: string) => {
    ...
    for (let positionIndex = 0; positionIndex < doctorRuntime.positions.length; positionIndex += 1) {
      ...
      const trailingActivationThresholdPct = Math.max(5, Math.min(positionTakeProfitPct, trailingActivationPct));
      const drawdownFromPeakPct = peakPrice > 0 && currentPrice > 0
      ...
      } else if (pnlPct >= trailingActivationThresholdPct && drawdownFromPeakPct >= positionTrailingStopPct) {
    ...
    const maxTokenAgeSeconds = Math.max(
      DOCTOR_MIN_WATCH_SECONDS,
      Math.min(DOCTOR_MAX_TOKEN_AGE_MINUTES, Math.trunc(getDoctorEffectiveControlNumber("max_token_age_minutes", 40))) * 60,
    );
  ...
  const buildDoctorStatus = async (userId?: string) => {
    ...
    return {
      ...
      trade_controls: {
        ...
        min_liquidity_usd: Math.max(
          DOCTOR_MIN_LIQUIDITY_USD,
          getDoctorEffectiveControlNumber("min_liquidity_usd", Number(doctorRuntime.controls.min_liquidity_usd || 0)),
        ),
        max_token_age_seconds: Math.max(
          DOCTOR_MAX_TOKEN_AGE_SECONDS,
          getDoctorEffectiveControlNumber("max_token_age_seconds", Number((doctorRuntime.controls as any).max_token_age_seconds || 0)),
        ),
        min_token_age_minutes: Math.max(
          DOCTOR_MIN_WATCH_SECONDS / 60,
  ...
  app.post("/api/doctor/config", isAuthenticated, async (req: any, res) => {
    ...
    doctorRuntime.controls.max_token_age_minutes = Math.min(
      DOCTOR_MAX_TOKEN_AGE_MINUTES,
      Math.max(Number(doctorRuntime.controls.min_token_age_minutes || 0), Number(doctorRuntime.controls.max_token_age_minutes || 40)),
    );
