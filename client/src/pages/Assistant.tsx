...
import { useAssistantContextOverview, useAskAssistant, useAssistDecision } from "@/hooks/use-ai-assistant";
...
export default function AssistantPage() {
  ...
  const askAssistant = async () => {
    ...
    if (!text) {
      toast({ title: "Question required", description: "Enter a question for DoctorStrange.", variant: "destructive" });
      return;
    }
    try {
      await askMutation.mutateAsync({ question: text });
    } catch (error) {
      toast({ title: "DoctorStrange error", description: error instanceof Error ? error.message : "Request failed", variant: "destructive" });
    }
  };
      ...
      <div className="space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
          <div className="space-y-1.5">
            <h1 className="text-3xl font-bold flex items-center gap-2">
              ...
              <span className="doctorstrange-font text-gradient">DoctorStrange</span>
            </h1>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          <Card className="solana-card">
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2"><MessageSquare className="w-4 h-4" />Ask DoctorStrange</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <Textarea
                value={question}
                onChange={(e) => setQuestion(e.target.value)}
                placeholder="Ask DoctorStrange about market risk, chain conditions, or trade strategy..."
        ...
        <Card className="solana-card">
          ...
          <CardContent className="space-y-3">
            {sortedChains.length === 0 ? (
              <p className="text-sm text-muted-foreground">No trade history yet. DoctorStrange will learn as trading data accumulates.</p>
            ) : (
